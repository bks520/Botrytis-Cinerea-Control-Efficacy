...
hist = model.fit(train_x_reshaped, train_y, epochs=100, batch_size=32, validation_data=(validation_x_reshaped, validation_y),
...
# 使用 type 函数检查数据类型
data_type = type(validation_x_reshaped)

if data_type == np.ndarray:
    print("validation_x_reshaped 是 NumPy 数组")
else:
    print("validation_x_reshaped 不是 NumPy 数组")


# callbacks=[early_stopping]
explainer = shap.Explainer(model, train_x_reshaped)
sample_index = 0  # 选择要解释的样本索引
shap_values = explainer.shap_values(train_x_reshaped[sample_index])
...
shap.summary_plot(shap_values, features=train_x_reshaped[sample_index], feature_names=["浓度梯度", "光照", "温度","营养"])
