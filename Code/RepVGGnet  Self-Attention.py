...
def RepVGGBlock(filters, kernel_size, strides=1, padding='valid', dilation=1, groups=1, deploy=False, use_se=False):
    def _RepVGGBlock(inputs):
        if deploy:
            if use_se:
                x = SelfAttention_block(inputs)  # 使用Self-Attention模块
                x = Conv2D(filters=filters, kernel_size=kernel_size, strides=strides,
                ...
                x = SelfAttention_block(x)  # 使用Self-Attention模块
                x = Activation('relu')(x)
            return x
...
class GradientClip(Adam):
    def __init__(self, clip_norm=1.0, **kwargs):
        self.clip_norm = clip_norm
        super(GradientClip, self).__init__(**kwargs)

    def get_gradients(self, loss, params):
        grads = super(GradientClip, self).get_gradients(loss, params)
        clipped_grads, _ = tf.clip_by_global_norm(grads, clip_norm=self.clip_norm)
        return clipped_grads

optimizer = GradientClip(clip_norm=1.0, learning_rate=learning_rate)
model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mae'])
...
with pd.ExcelWriter('evaluation_metrics3.xlsx', engine='xlsxwriter') as writer:
