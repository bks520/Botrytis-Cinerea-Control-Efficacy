...
def CBAM_block(x, reduction_ratio=16):
    ...
    avg_conv = Conv2D(filters=channels // reduction_ratio, kernel_size=1, strides=1, padding='same')(avg_pool)
    max_conv = Conv2D(filters=channels // reduction_ratio, kernel_size=1, strides=1, padding='same')(max_pool)
    avg_fc = Dense(units=channels, activation='relu')(avg_conv)
    max_fc = Dense(units=channels, activation='relu')(max_conv)
    channel_attention = Add()([avg_fc, max_fc])
    ...
    avg_pool = GlobalAveragePooling2D()(x)
    max_pool = GlobalMaxPooling2D()(x)
    spatial_attention = Add()([avg_pool, max_pool])
    spatial_attention = Activation('sigmoid')(spatial_attention)
    spatial_attention = Reshape((1, 1, channels))(spatial_attention)
...
def RepVGGBlock(filters, kernel_size, strides=1, padding='valid', dilation=1, groups=1, deploy=False, use_se=False):
    def _RepVGGBlock(inputs):
        if deploy:
            if use_se:
                x = CBAM_block(inputs)  # 使用CBAM模块替代SE模块
                x = Conv2D(filters=filters, kernel_size=kernel_size, strides=strides,
