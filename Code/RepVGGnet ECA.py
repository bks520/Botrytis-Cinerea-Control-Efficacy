...
def ECA_block(x, gamma=2, b=1):
    ...
    conv = Conv2D(filters=1, kernel_size=1, strides=1, padding='same')(avg_pool)
    conv = Activation('sigmoid')(b + conv * gamma)
    x = Multiply()([x, conv])
    return x
...
def RepVGGBlock(filters, kernel_size, strides=1, padding='valid', dilation=1, groups=1, deploy=False, use_se=False):
    def _RepVGGBlock(inputs):
        if deploy:
            if use_se:
                x = ECA_block(inputs)  # 使用ECA模块替代SE模块
                x = Conv2D(filters=filters, kernel_size=kernel_size, strides=strides,
...
hist = model.fit(train_x_reshaped, train_y, epochs=5, batch_size=32, validation_data=(validation_x_reshaped, validation_y),
...
with pd.ExcelWriter('evaluation_metrics1.xlsx', engine='xlsxwriter') as writer:
