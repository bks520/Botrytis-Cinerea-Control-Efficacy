...
patience = 100  # 设置您的耐心值，即在多少个epoch内没有性能改善时停止训练
early_stopping = EarlyStopping(monitor='val_loss', patience=patience, verbose=1)

hist = model.fit(train_x_reshaped, train_y, epochs=50, batch_size=32, validation_data=(validation_x_reshaped, validation_y),
...
validation_predictions = predict_with_model(model, validation_x_reshaped)
...
# Calculate predictions on validation set
validation_predictions = model.predict(validation_x_reshaped)

# Calculate Maximum Absolute Error
max_absolute_error = np.max(np.abs(validation_predictions - validation_y))
print("Maximum Absolute Error:", max_absolute_error)

# Calculate R-squared
r_squared = r2_score(validation_y, validation_predictions)
print("Coefficient of Determination (R-squared):", r_squared)

# Add Maximum Absolute Error and R-squared to evaluation_df DataFrame
evaluation_df['Max Absolute Error'] = max_absolute_error
evaluation_df['R-squared'] = r_squared

# Save DataFrame to Excel file
with pd.ExcelWriter('evaluation_metrics50.xlsx', engine='xlsxwriter') as writer:
