...
# y = df['MEDV']
# 载入自己的数据集
data = pd.read_csv("C:\\Users\\zxs\\PycharmProjects\\transtab-main\\embedding_output\\output1.csv")

# 获取特征数据和目标变量数据
X = data.iloc[:, :-1]  # 所有列除了最后一列为特征数据
y = data.iloc[:, -1]   # 最后一列为目标变量数据
...
# 绘制训练过程中的损失值和 R^2 的曲线图
plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.plot(range(0, num_epochs, 100), losses, 'b-')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training Loss')

plt.subplot(1, 2, 2)
plt.plot(range(0, num_epochs, 100), r2_values, 'g-')
plt.xlabel('Epoch')
plt.ylabel('R^2')
plt.title('R^2 Value')
